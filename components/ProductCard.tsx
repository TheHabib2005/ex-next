...
const ProductCard: FC<Props> = ({ item, setError }) => {
    ...
    return (
        <div className='mb-5'>
            ...
            {
                my(item.id) ? <button className='bg-blue-500 text-white p-2  disabled:bg-gray-300 rounded-md  mt-3' disabled={loading} onClick={() => removeCart(item)}>{loading ? "Loading" : "remove to Cart"}</button> : <button className='bg-blue-500 text-white p-2  disabled:bg-gray-300 rounded-md  mt-3' disabled={loading} onClick={() => handleCart(item)}>{loading ? "Loading" : "Add to Cart"}</button>
            }
        </div>
    )
}
